      ...
      connection.query(checkUser, [username], (error, result) => {
        if (error) throw error;
          ...
          if (!active) {
            ...
            connection.query(updateStatus, [oldActive, username], (error, result) => {
              if (error) throw error;
              console.log("NO CHANGE IN STATUS . . .");
            });
          } else {
            updateStatus = `UPDATE accounts SET status = ?, timestamp = NOW() WHERE username = ?`;
            connection.query(updateStatus, [active, username], (error, result) => {
              if (error) throw error;
                          ...
                          connection.query(checkCompositeKey, [username, group], (error, result) => {
                            if (error) throw error;
                              ...
                              connection.query(addCompositeKey, [group, username], (error, result) => {
                                if (error) throw error;
                                console.log(`NEW COMPOSITE KEY OF ${username} AND ${group} FOUND`);
